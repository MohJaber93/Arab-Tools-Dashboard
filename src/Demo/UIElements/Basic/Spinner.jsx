...
const BasicSpinner = () => {
    ...
    const basicSpinnerBorder = spinnerVariants.map((variant, idx) => (<div key={idx} className={['spinner-border', 'mr-1', 'text-' + variant].join(' ')} role="status">
            <span className="sr-only">Loading...</span>
        </div>));
    const basicSpinnerGrowing = spinnerVariants.map((variant, idx) => (<div key={idx} className={['spinner-grow', 'mr-1', 'text-' + variant].join(' ')} role="status">
            <span className="sr-only">Loading...</span>
        </div>));
    return (<>
            <Row>

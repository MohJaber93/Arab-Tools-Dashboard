...
const BasicAlert = () => {
    ...
    const basicAlert = alertVariants.map((variant, idx) => (<Alert key={idx} variant={variant}>
            A simple {variant} alert—check it out!
        </Alert>));
    const linkAlert = alertVariants.map((variant, idx) => (<Alert key={idx} variant={variant}>
            A simple {variant} alert with <Alert.Link href={DEMO.BLANK_LINK}>an example link</Alert.Link>. Give it a click if you like.
        </Alert>));
    const dismissingAlert = alertVariants.map((variant, idx) => (<Alert dismissible={true} key={idx} variant={variant}>
